ВНИМАНИЕ! Реально работает только javascript версия и немного esp8266. Остальное только в зачаточном состоянии.
...
в интернете. Кое что придется подчерпнуть и из соседних описаний (например формат записи в базе forms). 
То есть реализовано из чистого любопытства, безо всякой нужды. Вообще, прикольно видеть Json в языке 1964 года :) еще прикольнее 
видеть в нем Unicode c русскими именами команд и строк. 
Реализация вроде как "обратно совместимая", то есть все исходные программы по идее на ней должны работать.
...
- пока не реализованы диагностические функции - не придумал еще как удобнее сделать трассировку
