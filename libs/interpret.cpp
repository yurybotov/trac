...
void Trac::runstdtrac(int start) {
	...
	if(first[2] == 0) {
	  switch(first[0]) {
		  ...
		  case (litera)'o':
				switch(first[1]){
					case (litera)'r': or(f); free(f); free(first); return;
					default: break;
				}
			break;
		  case (litera)'p':
		  ...
		  case (litera)'u':
				switch(first[1]){
					case (litera)'n': un(f); free(f); free(first); return;
					case (litera)'s': us(f); free(f); free(first); return;
					default: break;
				}
			break;
		  case (litera)'x':
...
void Trac::runexttrac(litera* first, litera* f) {
  if(first[1] == 0) {
		...
		if(first[0] == (litera)'|') { or(f); free(f); free(first); return;}
